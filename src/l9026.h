...
// Prefix increment operator for OutputChannel
inline OutputChannel& operator++(OutputChannel& channel) {
  channel = static_cast<OutputChannel>(static_cast<uint8_t>(channel) + 1);
  return channel;
}

// Postfix increment operator for OutputChannel
inline OutputChannel operator++(OutputChannel& channel, int) {
  OutputChannel temp = channel;
  ++channel;
  return temp;
}
...
struct L9026Device {
  ...
  L9026Device(uint8_t id, int32_t in0_pin, int32_t in1_pin, int32_t idle_pin, int32_t reset_pin = -1, int32_t disable_pin = -1) :
    in0(in0_pin),
    in1(in1_pin),
    idle(idle_pin),
    reset(reset_pin),
    ...
    tx_buffer{0, 0},
    rx_buffer{0, 0},
    led_pwm_config(nullptr),
    general_pwm_config(nullptr),
    registers{0},
    transaction{0, 0, false, false, false},
...
class L9026 {
  ...
  public:
    L9026(SPIClass *spi, uint32_t cs, L9026Device devices[], uint8_t device_count, std::function<void(void)> take_spi = nullptr, std::function<void(void)> release_spi = nullptr) : spi(spi), cs(cs), devices(devices), device_count(device_count), take_spi(take_spi), release_spi(release_spi) {};
    bool begin();
    void end();
    bool getChipID();
    ...
    // Hardware reset pin control (NRES - active low, VFQFPN32 only)
    void assertReset(uint8_t device_id);
    void deassertReset(uint8_t device_id);

    // Hardware disable pin control (DIS - active high, VFQFPN32 only)
    ...
    // clear overcurrent/overtemperature error
    void clearAllDeviceOvercurrentOvertemperatureErrors();
    // void clearDeviceOvercurrentOvertemperatureError(uint8_t device_id, uint8_t channel_mask);
